...
func (aq *ArtistQuery) QueryEvents() *EventQuery {
...
func (aq *ArtistQuery) WithEvents(opts ...func(*EventQuery)) *ArtistQuery {
	query := (&EventClient{config: aq.config}).Query()
...
func (aq *ArtistQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Artist, error) {
	...
	if query := aq.withEvents; query != nil {
		if err := aq.loadEvents(ctx, query, nodes,
			func(n *Artist) { n.Edges.Events = []*Event{} },
			func(n *Artist, e *Event) { n.Edges.Events = append(n.Edges.Events, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ArtistQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Artist, init func(*Artist), assign func(*Artist, *Event)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(artist.EventsTable)
		s.Join(joinT).On(s.C(event.FieldID), joinT.C(artist.EventsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(artist.EventsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(artist.EventsPrimaryKey[1]))
